...
extension CVCalendarContentViewController {
    public func dateBeforeDate(_ date: Foundation.Date) -> Foundation.Date {
        ...
        let dateBefore = (calendar as NSCalendar).date(byAdding: components, to: date, options: NSCalendar.Options.matchPreviousTimePreservingSmallerUnits)!
        return dateBefore
    }
    ...
    public func dateAfterDate(_ date: Foundation.Date) -> Foundation.Date {
        ...
        components.month = 1
        let calendar = Calendar.current
        let dateAfter = (calendar as NSCalendar).date(byAdding: components, to: date, options: NSCalendar.Options.matchPreviousTimePreservingSmallerUnits)!
        return dateAfter
    }
